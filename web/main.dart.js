...
(function dartProgram() {
  ...
  P = {
    ...
    _iterablePartsToStrings: function(iterable, parts) {
      var cur, next, ultimateString, penultimateString, penultimate, ultimate, elision,
        it = new H.ListIterator(iterable, iterable._source.length, iterable.$ti._eval$1("ListIterator<ListIterable.E>")),
        $length = 0, count = 0;
      while (true) {
      ...
      if (!it.moveNext$0()) {
        ...
        if (!it.moveNext$0()) {
          ...
          ++count;
          for (; it.moveNext$0(); penultimate = ultimate, ultimate = cur) {
            cur = it.__internal$_current;
            ++count;
            if (count > 100) {
              while (true) {
  ...
  K = {
    Cells_nextGeneration: function(state, rules) {
      var new_state, i, chunk, t1, rule, _box_0 = {},
        configurations = H.setRuntimeTypeInfo(["111", "110", "101", "100", "011", "010", "001", "000"], type$.JSArray_legacy_String);
      _box_0.config = null;
      new_state = H.setRuntimeTypeInfo([], type$.JSArray_legacy_int);
      state = P.List_List$from(state, type$.dynamic);
      C.JSArray_methods.add$1(state, 0);
      C.JSArray_methods.insert$2(state, 0, 0);
      for (i = 0; i < state.length - 2; ++i) {
        chunk = C.JSArray_methods.sublist$2(state, i, i + 3);
        t1 = H._arrayInstanceType(chunk);
        _box_0.config = new H.MappedListIterable(chunk, t1._eval$1("String*(1)")._as(new K.Cells_nextGeneration_closure()), t1._eval$1("MappedListIterable<1,String*>")).join$1(0, "");
        rule = C.JSArray_methods.indexWhere$1(configurations, new K.Cells_nextGeneration_closure0(_box_0));
        if (rule < 0 || rule >= rules.length)
          return H.ioore(rules, rule);
        C.JSArray_methods.add$1(new_state, rules[rule]);
      }
      return new_state;
    },
    Cells_generate: function(gen_number, rule_number) {
      var gen_size, rules, state, i, generations, t1, new_state;
      if (typeof gen_number !== "number")
        return gen_number.$mul();
      gen_size = gen_number * 2 + 1;
      rules = K.Cells_numberToBinary(rule_number);
      state = H.setRuntimeTypeInfo([], type$.JSArray_legacy_int);
      for (i = 0; i < gen_size; ++i)
        C.JSArray_methods.add$1(state, 0);
      C.JSArray_methods.$indexSet(state, gen_number, 1);
      K.Cells_nextGeneration(state, rules);
      generations = H.setRuntimeTypeInfo([state], type$.JSArray_legacy_List_legacy_int);
      for (t1 = type$.legacy_int, i = 0; i < gen_number; ++i) {
        new_state = K.Cells_nextGeneration(state, rules);
        C.JSArray_methods.add$1(generations, new_state);
        state = P.List_List$from(new_state, t1);
      }
      return generations;
    },
    Cells_numberToBinary: function(rule_number) {
      var pad_amount, i,
        binary_number = H.setRuntimeTypeInfo([], type$.JSArray_legacy_int);
      while (true) {
        if (typeof rule_number !== "number")
          return rule_number.$gt();
        if (!(rule_number > 0))
          break;
        C.JSArray_methods.insert$2(binary_number, 0, C.JSInt_methods.$mod(rule_number, 2));
        rule_number = C.JSInt_methods._tdivFast$1(rule_number, 2);
      }
      pad_amount = 8 - binary_number.length;
      for (i = 0; i < pad_amount; ++i)
        C.JSArray_methods.insert$2(binary_number, 0, 0);
      return binary_number;
    },
    Cells_nextGeneration_closure: function Cells_nextGeneration_closure() {
    },
    Cells_nextGeneration_closure0: function Cells_nextGeneration_closure0(t0) {
      this._box_0 = t0;
    }
  },
  F = {
    main: function() {
      var t1 = document,
        ...
        size_element = t2._as(t1.querySelector("#size")),
        rule_element = t2._as(t1.querySelector("#rule")),
        cell_size_element = t2._as(t1.querySelector("#cell-size")),
        submit_button = type$.legacy_ButtonElement._as(t1.querySelector("#submit_button"));
      t1 = type$.legacy_CanvasElement._as(t1.querySelector("#mycanvas"));
      $.canvas = t1;
      $.context = type$.legacy_CanvasRenderingContext2D._as((t1 && C.CanvasElement_methods).getContext$1(t1, "2d"));
      submit_button.toString;
      t1 = type$._ElementEventStreamImpl_legacy_MouseEvent;
      t2 = t1._eval$1("~(1)?")._as(new F.main_closure(rule_element, size_element, cell_size_element));
      type$.nullable_void_Function._as(null);
      W._EventStreamSubscription$(submit_button, "click", t2, false, t1._precomputed1);
    },
    generate: function(size, rule, cell_size) {
      var t2, y, i, row, x, k,
        grid = K.Cells_generate(size, rule),
        t1 = $.canvas;
      if (0 >= grid.length)
        return H.ioore(grid, 0);
      t2 = grid[0].length;
      if (typeof cell_size !== "number")
        return H.iae(cell_size);
      (t1 && C.CanvasElement_methods).set$width(t1, t2 * cell_size);
      t2 = $.canvas;
      (t2 && C.CanvasElement_methods).set$height(t2, grid.length * cell_size);
      t2 = $.context;
      t1 = $.canvas;
      t2.clearRect(0, 0, t1.width, t1.height);
      for (y = 0, i = 0; i < grid.length; ++i) {
        row = grid[i];
        for (x = 0, k = 0; k < row.length; ++k) {
          if (row[k] === 1)
    ...
    main_closure: function main_closure(t0, t1, t2) {
      this.rule_element = t0;
      this.size_element = t1;
      this.cell_size_element = t2;
    }
  };
  var holders = [C, H, J, P, W, K, F];
  ...
  H.ListIterable.prototype = {
    join$1: function(_, separator) {
      var t2, first, t3, t4, i, t0, _this = this,
        t1 = _this._source,
        $length = t1.length;
      if (separator.length !== 0) {
        if ($length === 0)
        ...
        for (t2 = _this._f, t3 = $length, i = 0, t4 = ""; i < $length; ++i, t0 = t4, t4 = t3, t3 = t0) {
          if (i >= t3)
            return H.ioore(t1, i);
          t3 = t4 + H.S(t2.call$1(t1[i]));
          t4 = t1.length;
          if ($length !== t4)
            throw H.wrapException(P.ConcurrentModificationError$(_this));
        }
  ...
  P.Iterable.prototype = {
    get$length: function(_) {
      var count,
        it = new H.ListIterator(this, this._source.length, this.$ti._eval$1("ListIterator<ListIterable.E>"));
  ...
  K.Cells_nextGeneration_closure.prototype = {
    call$1: function(bit) {
      return J.toString$0$(bit);
    },
    $signature: 11
  };
  K.Cells_nextGeneration_closure0.prototype = {
    call$1: function(item) {
      return H._asStringS(item) == this._box_0.config;
    },
    $signature: 12
  };
  F.main_closure.prototype = {
    call$1: function($event) {
      ...
      rule = P.int_parse(this.rule_element.value);
      F.generate(P.int_parse(this.size_element.value), rule, P.int_parse(this.cell_size_element.value));
    },
    $signature: 13
  };
  (function aliases() {
  ...
  (function inheritance() {
    ...
    _inheritMany(W.HtmlElement, [W.AnchorElement, W.AreaElement, W.ButtonElement, W.CanvasElement, W.FormElement, W.InputElement, W.SelectElement]);
  ...
  var init = {
    ...
    metadata: [],
    types: ["Null()", "~()", "~(~())", "Null(@)", "@(@)", "@(@,String)", "@(String)", "Null(~())", "Null(Object,StackTrace)", "_Future<@>(@)", "@(Event)", "String*(@)", "bool*(String*)", "Null(MouseEvent*)"],
    interceptorsByTag: null,
    leafTags: null,
    arrayRti: typeof Symbol == "function" && typeof Symbol() == "symbol" ? Symbol("$ti") : "$ti"
  };
  H._Universe_addRules(init.typeUniverse, JSON.parse('{"PlainJavaScriptObject":"JavaScriptObject","UnknownJavaScriptObject":"JavaScriptObject","JavaScriptFunction":"JavaScriptObject","AbortPaymentEvent":"Event","ExtendableEvent":"Event","AElement":"Element","GraphicsElement":"Element","SvgElement":"Element","AudioElement":"HtmlElement","MediaElement":"HtmlElement","PointerEvent":"MouseEvent","CompositionEvent":"UIEvent","HtmlDocument":"Node","Document":"Node","JSBool":{"bool":[]},"JSNull":{"Null":[]},"JavaScriptObject":{"Function":[]},"JSArray":{"List":["1"],"Iterable":["1"]},"JSUnmodifiableArray":{"JSArray":["1"],"List":["1"],"Iterable":["1"]},"JSNumber":{"num":[]},"JSInt":{"int":[],"num":[]},"JSDouble":{"num":[]},"JSString":{"String":[]},"EfficientLengthIterable":{"Iterable":["1"]},"ListIterable":{"Iterable":["1"]},"MappedListIterable":{"ListIterable":["2"],"Iterable":["2"],"ListIterable.E":"2"},"NullError":{"Error":[]},"JsNoSuchMethodError":{"Error":[]},"UnknownJsTypeError":{"Error":[]},"_StackTrace":{"StackTrace":[]},"Closure":{"Function":[]},"TearOffClosure":{"Function":[]},"StaticClosure":{"Function":[]},"BoundClosure":{"Function":[]},"RuntimeError":{"Error":[]},"_AssertionError":{"Error":[]},"_Error":{"Error":[]},"_TypeError":{"Error":[]},"_Future":{"Future":["1"]},"AsyncError":{"Error":[]},"_Zone":{"Zone":[]},"_RootZone":{"_Zone":[],"Zone":[]},"int":{"num":[]},"List":{"Iterable":["1"]},"AssertionError":{"Error":[]},"TypeError":{"Error":[]},"NullThrownError":{"Error":[]},"ArgumentError":{"Error":[]},"RangeError":{"Error":[]},"IndexError":{"Error":[]},"UnsupportedError":{"Error":[]},"UnimplementedError":{"Error":[]},"ConcurrentModificationError":{"Error":[]},"StackOverflowError":{"Error":[]},"CyclicInitializationError":{"Error":[]},"_StringStackTrace":{"StackTrace":[]},"HtmlElement":{"Element":[],"EventTarget":[]},"AnchorElement":{"Element":[],"EventTarget":[]},"AreaElement":{"Element":[],"EventTarget":[]},"ButtonElement":{"Element":[],"EventTarget":[]},"CanvasElement":{"Element":[],"EventTarget":[]},"Element":{"EventTarget":[]},"FormElement":{"Element":[],"EventTarget":[]},"InputElement":{"Element":[],"EventTarget":[]},"MouseEvent":{"Event":[]},"Node":{"EventTarget":[]},"SelectElement":{"Element":[],"EventTarget":[]},"UIEvent":{"Event":[]},"_EventStream":{"Stream":["1"]},"_ElementEventStreamImpl":{"_EventStream":["1"],"Stream":["1"]}}'));
  H._Universe_addErasedTypes(init.typeUniverse, JSON.parse('{"EfficientLengthIterable":1,"StreamSubscription":1}'));
  0;
  var type$ = (function rtii() {
    ...
    return {
      ...
      int: findType("int"),
      legacy_ButtonElement: findType("ButtonElement*"),
